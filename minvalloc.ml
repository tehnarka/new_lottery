...
let minvalloc (input : (string * int) list) (blocks_per_cycle : int) : string array =
  ...
  let stakes = Array.of_list (List.map snd input) in
  let norm = Array.map (fun s -> float s /. float total_stake) stakes in

  let entitlements = Array.map (fun s -> float blocks_per_cycle *. s) norm in

  let cE = Array.make (n + 1) 0.0 in
  for i = 0 to n - 1 do
    cE.(i + 1) <- cE.(i) +. entitlements.(i)
  done;
  ...
  for i = 0 to n - 1 do
    let left = floor (u +. cE.(i)) in
    let right = floor (u +. cE.(i + 1)) in
    slots.(i) <- int_of_float (right -. left)
  done;
  ...
  for i = 0 to n - 1 do
    for _ = 1 to slots.(i) do
      if !idx < blocks_per_cycle then (
        result.(!idx) <- delegates.(i);
        incr idx
      )
    done
  done;
...
let verify_allocation (input : input) (output : output) (blocks_per_cycle : int) : unit =
  ...
  (* Count how many blocks each delegate actually received *)
  ...
  (* Sort input for consistent and readable output *)
  ...
  (* For each delegate, compute expected vs. actual and print *)
...
let output_computed = minvalloc input_example blocks_per_cycle
